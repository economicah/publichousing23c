...
Histogram of permutation test, with observed difference displayed by the red line:
```{r echo = FALSE, results = "asis", message = FALSE}
hist(diff)
...
```
<br><br>According to our permutation test, there's a 95% chance of the difference in mean waiting times
...
<br><br><br><br>
Then, we looked at the difference in mean average waiting time of poor areas vs non-poor areas. We did this both through a permutation test and a t.test. 
<br>
*Permutation Test*
...
t_apa(t_test(hcv$months_waiting ~ hcv$poverty_area, hcv), format = "html", info = FALSE, print = TRUE)
```
<br><br>According to our permutation test, there's a 95% chance of the difference in mean waiting times between poor and non poor areas occurring by chance. However, according to the t-test, there's only a 12% chance of this occurring by chance. The permutation test is superior to the t-test and this difference in means is definitely not significant.
